...
export function ProductCard({ product, onToggleFavorite }: ProductCardProps) {
  ...
  return (
    <Card className="border-warm-brown/20 hover:shadow-lg transition-all duration-300 group overflow-hidden rounded-lg">
      <div className="relative">
        ...
        <div className="absolute top-3 left-3">
          <Badge className="bg-heritage-gold text-deep-teal text-xs font-medium shadow-sm">{product.authenticity_score}% Authentic</Badge>
        </div>

        {/* Category Badge */}
        <div className="absolute bottom-3 left-3">
          <Badge className="bg-white/90 text-deep-teal text-xs font-medium shadow-sm">{product.category}</Badge>
        </div>
      </div>
      ...
      <CardContent className="p-5 space-y-4">
        {/* Product Name */}
        <Link href={`/product/${product.id}`}>
          <h3 className="font-semibold text-deep-teal line-clamp-2 hover:text-heritage-gold transition-colors text-lg">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="flex items-center space-x-2">
          <img
            ...
            className="w-7 h-7 rounded-full object-cover border border-heritage-gold/30"
          />
        ...
        <div className="flex flex-wrap gap-1.5">
          {product.heritage_tags.slice(0, 2).map((tag) => (
            <Badge key={tag} variant="outline" className="text-xs border-sunset-orange/30 text-sunset-orange px-2 py-1">
              {tag}
            </Badge>
          ))}
          {product.heritage_tags.length > 2 && (
            <Badge variant="outline" className="text-xs border-warm-brown/30 text-warm-brown px-2 py-1">
              +{product.heritage_tags.length - 2}
            </Badge>
          )}
        </div>
        ...
        <Button 
          ...
          className="w-full bg-heritage-gold text-deep-teal hover:bg-heritage-gold/90 shadow-sm transition-all duration-200 hover:shadow-md"
        >
